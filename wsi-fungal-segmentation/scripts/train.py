...
def train_one_epoch(
    ...
    Returns dict with avg loss, dice, iou, density_loss for the epoch.
    """
    model.train()
    ...
    return {
        ...
        "density_loss":  running_density_loss / denom,
...
def evaluate(
    ...
    Expects loader to yield (imgs, masks, density_labels).
    Returns dict with avg loss, dice, iou, precision, recall, density_loss — or None.
    """
    if loader is None:
        return None
...
def _run_training(cfg: dict, progress_file: str | None = None):
    ...
    if resume_path.exists():
        ...
        best_val_dice    = ckpt.get("best_dice", -1.0)
        history          = ckpt.get("history", history)
        no_improve_count = 0
        log(f"  Resumed — best dice so far: {best_val_dice:.4f}, "
            f"history length: {len(history['train_loss'])} epochs")
    else:
